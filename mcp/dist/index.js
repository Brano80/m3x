...
Returns: all 17 markets with slug, label, and who each side is for.`,
    ...
    const lines = [
        ...
        '',
        ...markets.map(m => `• ${m.slug.padEnd(20)} ${m.label}\n  demand: ${m.demand}\n  supply: ${m.supply}`),
        '',
...
Markets: venture_capital, ma_deal_flow, real_estate, private_equity, b2b_saas,
    ...
    try {
        ...
        const result = await fetch(`${M3X_API_URL}/markets/${slug}/template`);
        const data = await result.json();
        if (!data.template)
            return { content: [{ type: "text", text: JSON.stringify(data, null, 2) }] };
        const side = data.template[params.side];
        const questions = side.interview;
        ...
        const lines = [
            ...
            ...questions.map((q, i) => `${i + 1}. [${q.required ? 'REQUIRED' : 'optional'}] ${q.question}${q.example ? `\n   Example: "${q.example}"` : ''}`),
            ``,
