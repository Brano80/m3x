# Removes the library sidebar (full-width list + one-line footer API hint). Build-gated push.

$candidates = @(
    "C:\Program Files\Git\cmd\git.exe","C:\Program Files\Git\bin\git.exe",
    "C:\Program Files (x86)\Git\cmd\git.exe","$env:LOCALAPPDATA\Programs\Git\cmd\git.exe",
    "$env:USERPROFILE\scoop\apps\git\current\bin\git.exe","C:\ProgramData\chocolatey\bin\git.exe",
...
if ($LASTEXITCODE -ne 0) {
    Write-Host "`nBUILD FAILED - nothing committed or pushed. Tell Claude." -ForegroundColor Red
    Read-Host "Press Enter to close"; exit 1
}

Write-Host "`nStep 2/3: staging the 3 files..." -ForegroundColor Cyan
& $git add "app/library/LibraryClient.tsx" "app/library/page.module.css" "app/library/page.tsx"
& $git diff --cached --stat

$confirm = Read-Host "`nStep 3/3: commit and push? (y/n)"
if ($confirm -ne 'y') { Write-Host "Aborted." -ForegroundColor Yellow; exit 0 }

& $git commit -m "ux: remove library sidebar panels - full-width list, api hint moved to footer"
